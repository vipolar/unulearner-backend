...
public class StorageTaskCreateDirectory extends StorageTask {
    public StorageTaskCreateDirectory(StorageTree storageTree, StorageTreeNode targetNode, StorageTreeNode destinationNode, StorageTasksMap storageTasksMap) {
        ...
        this.setTaskHeading("Create %s directory in %s".formatted(this.getRootTarget().getOnDiskName(), this.getRootDestination().getOnDiskURL()));
        this.setTaskCurrentState("Directory creation task was initiated...".formatted(), null, StateCode.RUNNING);
    }
    ...
    public synchronized void executeTask(Boolean skipOnException, Boolean skipOnExceptionIsPersistent, String onExceptionAction, Boolean onExceptionActionIsPersistent, Boolean cancelTaskExecution) {
        ...
        if (currentTarget == null) {
            this.setTaskCurrentState("Directory creation task finished successfully!".formatted(), null, StateCode.COMPLETED);
            this.cleanTaskUp();
            return;
        }

        if (cancelTaskExecution != null && cancelTaskExecution == true) {
            this.setTaskCurrentState("Directory creation task was cancelled...".formatted(), null, StateCode.CANCELLED);
            this.cleanTaskUp();
            return;
        }
        ...
        if (skipOnException != null) {
            ...
            if (this.getSkipOnException(currentTarget) == true && this.getAttemptCounter() > 0) {
                this.setTaskCurrentState("Directory '%s' creation in directory '%s' was skipped...".formatted(currentTarget.getOnDiskName(), currentDestination.getOnDiskURL()), null, StateCode.RUNNING);
                this.cleanTaskUp();
                this.advanceTask();
                return;
            }
        ...
        try {
            ...
            this.setTaskCurrentState("Directory '%s' was created in directory '%s' successfully!".formatted(currentTarget.getOnDiskName(), currentDestination.getOnDiskURL()), null, StateCode.RUNNING);
            this.cleanTaskUp();
            this.advanceTask();
            return;
        } catch (Exception exception) {
            if (this.getSkipOnException(currentTarget) == true && this.getAttemptCounter() > 0) {
                this.setTaskCurrentState("Directory '%s' creation in directory '%s' was skipped automatically...".formatted(currentTarget.getOnDiskName(), currentDestination.getOnDiskURL()), null, StateCode.RUNNING);
                this.cleanTaskUp();
                this.advanceTask();
                return;
            }
            ...
            if (this.getAttemptCounter() > 3) {
                this.setTaskCurrentState("Directory '%s' could not be created in directory '%s' - %s".formatted(currentTarget.getOnDiskName(), currentDestination.getOnDiskURL(), exception.getMessage()), null, StateCode.ERROR);
                this.cleanTaskUp();
            ...
            this.setTaskCurrentState("Directory '%s' could not be created in directory '%s' - %s".formatted(currentTarget.getOnDiskName(), currentDestination.getOnDiskURL(), exception.getMessage()), onExceptionOptions, StateCode.EXCEPTION);
            this.cleanTaskUp();
            return;
        }
    }
